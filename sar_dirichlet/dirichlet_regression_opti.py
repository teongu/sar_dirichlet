...
def objective_func_loglik_spatial(x, X, Y, W, Z=None, phi=None, regularization_lambda=0, epsilon=0, P=None, D=None, P_inv_X=None):
    K = X.shape[-1]
    J = Y.shape[-1]
    n = X.shape[0]
    beta = np.zeros((K,J))
...
def jac_objective_func_loglik_spatial(x, X, Y, W, Z=None, phi=None, regularization_lambda=0, epsilon=0, P=None, D=None, P_inv_X=None):
    K = X.shape[-1]
    J = Y.shape[-1]
    n = X.shape[0]
    beta = np.zeros((K,J))
        ...
        if self.spatial:
            if loss=='crossentropy':
                ...
                if parametrization=='common':
                    ...
                    min_bounds[-1], max_bounds[-1] = -1, 1
                    bounds = Bounds(min_bounds, max_bounds)
                    if W_is_diagonalizable:
                        solution = minimize(objective_func_loglik_spatial, params0, args=(X_f, Y, W, None, self.phi, regularization, 0, P, D, P_inv_X), bounds=bounds, options={'maxiter':self.maxiter, 'maxfun':self.maxfun})
                    else:
                    ...
                    bounds = Bounds(min_bounds, max_bounds)
                    if W_is_diagonalizable:
                        solution = minimize(objective_func_loglik_spatial, params0, args=(X_f, Y, W, Z, None, regularization, 0, P, D, P_inv_X), bounds=bounds, options={'maxiter':self.maxiter, 'maxfun':self.maxfun})
                    else:
